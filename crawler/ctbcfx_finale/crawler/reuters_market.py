...
class ReutersMarket_Crawler:
	def __init__(self, startpage=0, stoppage=None):
		...
		if stoppage != None:
			...
			self.startpage = 0
			self.stoppage = int(self.config['REUTERS']['LastPage_Market'])
			
	def get_url(self):
		url_list = []
		for i in range(self.startpage,self.stoppage+1):
			main_url = self.config['REUTERS']['QueryURL_Market'] + str(i) + '&pageSize=10'
			q = pq(main_url)
	...
	def execute(self, output_csv=True):
		...
		if output_csv:
			data.to_csv(self.config['REUTERS']['OutputPath_Market'],encoding = 'utf-8',index = None)
