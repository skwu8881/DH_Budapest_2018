...
raw_data = DBconn.query(cols='*',table='zerohedge',condition_str='time > "2017-10-20 00:00:00"')
...
market_path = pd.read_csv(r'C:\Users\z00013855\Desktop\treemodel\market.csv',encoding = 'utf-8')
                ...
                for z in indice:
                    if indice_sent[i] - min(ta) <= z <= indice_sent[i]+min(ta):
                        ...
                        if neg_type1 == True:
                            ...
                            '''
                        if neg_type1 == False:
                            if str(sent) not in temp_list:
                                print(str(sent))
                                '''
                                wfile.write('========================================'+'\n')
                                wfile.write(str(sent)+'\n')
                                '''
                                temp_list.append(str(sent))
                                ...
                                '''
                                for key,value in entity_match_dict.items():
                                    if EA == key:
                                        if sen_type in value:
...
time_frame = pd.DataFrame(fomc_news['time'].astype(str).str[:10],columns = ['time'])
time_frame = time_frame.reset_index(drop=True)
result = pd.concat([time_frame, ea_frame], axis=1) #,ea_marketFrame
complex_result = pd.concat([time_frame, eas_frame], axis=1) #,ea_marketFrame

complex_result.fillna(0,inplace=True)
complex_result.to_csv('complex_date_freq2.csv',encoding = 'utf-8')
bydate = complex_result.groupby('time').sum().reset_index()
bydate.to_csv('bydate.csv',encoding = 'utf-8')
...
print('總共分析',len(fomc_news),'篇新聞，花費',str(round(diff.seconds/60.0,2)),'分鐘')  #總共爬取 20000 個案子，花費 76.33 分鐘
